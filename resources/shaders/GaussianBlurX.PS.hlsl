...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    float uvStepSize = rcp((float)width);
    ...
    for (int i = 1; i <= 8; ++i) {
        ...
        float2 texcoordL = input.texcoord + float2(-i * uvStepSize, 0.0f);
        totalColor += gTexture.Sample(gSampler, texcoordL) * weight;
        totalWeight += weight;

        float2 texcoordR = input.texcoord + float2(i * uvStepSize, 0.0f);
        totalColor += gTexture.Sample(gSampler, texcoordR) * weight;
        totalWeight += weight;
    }
